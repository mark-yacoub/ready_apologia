        ...
        <div key={catIdx} style={{ marginBottom: '48px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {categorySection.variants.map((ex, i) => {
              const refMatch = ex.surah_ref ? ex.surah_ref.match(/\[quran\/(\d+)[\/:](\d+)\]/i) : null;
              const refUrl = refMatch ? getQuranUrl({ surah: refMatch[1], ayah: refMatch[2] }) : '#';
              const refLabel = refMatch ? `Quran ${refMatch[1]}:${refMatch[2]}` : ex.surah_ref;
